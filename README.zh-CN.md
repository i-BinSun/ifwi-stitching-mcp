...
一层很薄的 FastMCP 工具层背后是若干职责单一的模块。所有函数都返回**统一的结果结构** ——
`{"ok": True, "data": {...}}` 或 `{"ok": False, "error_code": <枚举>, "message": <str>, "detail": {...}}`。
任何裸异常都不会跨越工具边界。

| 模块 | 职责 | 约束 |
|------|------|------|
| `result.py` | `ok()` / `err()` 辅助函数 + `ErrorCode` 常量 | — |
| `config.py` | 环境变量、配置文件、缓存布局、启动校验、token 访问 | **唯一**读取 `os.environ` 和配置文件的模块 |
...
- **Python 3.9+**
- 依赖：`fastmcp`、`requests`、`py7zr`（运行时）；`pytest`、`responses`（开发/测试）
...
服务器读取三个环境变量。`FIV_BASE_URL` 在启动时必需；两个 token 是**懒校验**的，只有在调用到需要它们的工具时才检查。

| 变量 | 是否必需 | 用途 |
|------|----------|------|
| `FIV_BASE_URL` | **是** | FIV Portal 的基础 URL，例如 `https://fiv.example.com`。REST 路径拼接为 `<base>/app/rest/<endpoint>`。 |
| `FIV_TOKEN` | 调用 FIV 时 | FIV 访问 token，以 `Authorization: Bearer <token>` 发送。 |
| `ARTIFACTORY_TOKEN` | 下载时 | Artifactory 访问 token，下载产物时以 `Authorization: Bearer <token>` 发送。 |
| `IFWI_MCP_CACHE_DIR` | 否 | 缓存根目录，默认 `~/.ifwi-stitching-mcp/cache`。子目录：`ifwi/`、`ingredients/`、`stitch/`、`extracted/`、`work/`、`plans/`、`deliverables/`。 |
...
启动时会校验 `FIV_BASE_URL`、缓存目录可创建、以及 execution 配置段；失败时向 stderr 打印一条错误 JSON 并以非零
状态退出，否则以 stdio 方式启动 FastMCP 服务器。
...
`envFile` 让 token 集中放在 gitignore 掉的 `.env` 里，不出现在配置文件中；`dev.watch` 会在源码改动后自动重启
服务器（配合可编辑安装，改完保存即生效）。
...
去掉那三个 `IFWI_MCP_EXEC_*` 就是本地执行，也是默认行为。
...
| `fiv_find_ingredient` | `(project, name, version)` | 解析 ingredient 的下载 URL。 |
...
| `run_stitch` | `(stitch_dir, binary_file, ingredient_name, ingredient_path, config_ini, soft_strap=None)` | 在 venv 中运行 stitch 工具的 `cli.py`，返回最新产出的 `.bin`。 |
...
| `stitch_build_plan` | `(project, phase, ingredient_name, config_ini, version=None, swimlane=None, ifwi_binary=None, ifwi_path=None, ifwi_from_build_targets=False, ingredient_version=None, ingredient_path=None, stitch_version=None, stitch_path=None, soft_strap=None)` | 把与用户确认好的答案变成一份保存下来的 plan + 命令行。不下载任何东西。 |
...
**Plan 驱动流程（推荐）** —— 宿主先向用户提问，再把确认好的答案交过来：
...
2. `stitch_build_plan(...)` → 保存下来的 plan + `command_line`，例如：
   `{python} cli.py --binary_file {binary_file} --ingredient_name MMC1 --ingredient_path {ingredient_path} --config_ini {config_ini}`。
   花括号是占位符，对应那些要等环境准备完才存在的路径 —— 把这行展示给用户做最终确认。
...
6. `run_stitch(stitch_dir, binary_file, ingredient_name, ingredient_path, config_ini)` → stitch 后的 `.bin`
